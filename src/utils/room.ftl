...
<body>
    ...
    <div id="mainRadius" style="width: 600px;height:400px;"></div>
    ...
    <script>
      window.dataStr = '${data.tableList}';
      var tableList = [];
      <#if data.tableList??>
        <#list data.tableList as item>
             tableList.push({ value: "${item.dataQualityPercent}", name: "${item.tableEnName}"});
        </#list>
       </#if>
      window.onload = function () {
        console.log('>>>>>>>>>>>>>>>>>>>>>>',tableList,  dataStr, echarts);
        var myChart = echarts.init(document.getElementById('main'));
        var myRadiusChart = echarts.init(document.getElementById('mainRadius'));
        ...
        myRadiusChart.setOption(Radiuoption);
      };
